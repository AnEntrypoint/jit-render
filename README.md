...
- **Batteries Included** - 36 pre-built components (shadcn/ui for React, or dependency-free webjsx components)
...
| `jit-render`        | WebJSX renderer — no React, no Tailwind, Web Standards only            |
...
```bash
git clone https://github.com/AnEntrypoint/jit-render
cd jit-render
pnpm install
pnpm dev
```
